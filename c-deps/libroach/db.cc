...
DBStatus DBEnvReadFile(DBEngine* db, DBSlice path, DBSlice* contents, uint64_t size) {
  return db->EnvReadFile(path, contents, size);
}
...
DBStatus DBEnvDeleteDir(DBEngine* db, DBSlice path) {
  return db->EnvDeleteDir(path);
}
